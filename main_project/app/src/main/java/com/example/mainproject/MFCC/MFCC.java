package com.example.mainproject.MFCC;
...
public class MFCC {
    ...
    private final static int       n_fft                = 512;//400;//2048;
    private final static int       hop_length           = 128;//160;//512;
    private final static int	   n_mels               = 40;//40;//128;
    ...
    public float[][] process_2d(double [] doubleInputBuffer){
        final double[][] mfccResult = dctMfcc(doubleInputBuffer);
        float [][] mfccOut = new float[mfccResult.length][mfccResult[0].length];
        for(int i = 0; i<mfccResult.length;i++){
            for(int j = 0; j<mfccResult[0].length;j++){
                mfccOut[i][j] = (float) mfccResult[i][j];
            }
        }
        return mfccOut;
    }
